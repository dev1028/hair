				...
				Thread task = new Thread(new Runnable() {
					public void run() {
						...
						String contents = "<h3>디자이너 인증요청</h3>" + "<span><strong>" + hVo.getHs_owner()
								...
								+ "<a href='http://192.168.0.83/hairapp/ajax/hairshopEmailAuth.do?hs_email="+ hVo.getHs_email() + "'>인증완료</a>";
					...
					Thread task = new Thread(new Runnable() {
						public void run() {
							...
							String contents = "<h3>미용실, 디자이너  인증요청</h3>" + "<span><strong>" + hVo.getHs_owner()
									+ "</strong>님 회원가입을 축하합니다.</span><br>"
									+ "<a href='http://192.168.0.83/hairapp/ajax/hairshopEmailAuth.do?hs_email="+ hVo.getHs_email() + "'>미용실 인증완료</a>"
											+ "<br><a href='http://192.168.0.83/hairapp/ajax/employeeAuth.do?designer_email="+ hVo.getDesigner_email() + "'>디자이너 인증완료</a>";
							// 나중에 이메일 인증 ip주소는 웹서버껄로 바꿔야함
							em.setContents(contents);
							se.sand(em);
						}
					...
					Thread task = new Thread(new Runnable() {
						public void run() {
							...
							String contents2 = "<h3>디자이너 인증요청</h3>" + "<span><strong>" + hVo.getHs_owner()
									+ "</strong>님 회원가입을 축하합니다.</span><br>"
											+ "<a href='http://192.168.0.83/hairapp/ajax/employeeAuth.do?designer_email="+ hVo.getDesigner_email() + "'>인증완료</a>";
							// 나중에 이메일 인증 ip주소는 웹서버껄로 바꿔야함
